...
var apiCalls = [
    {name:"Newsfeed Post ID", type: "String", desc: "The ID created for the POST made by the user, if the newsfeed post ID and API Key are related, then the resource may be changed. Otherwise the request will be denied, this prevents users from deleting other users's posts", example: "13473458753" }];

var apiDocumentation = [
    {content: "Want to implement LAN's REST API into your own app? Doing so is easy, once you have registered for a LAN account, you can click the \"Get an API Key\" link located on the login page. This will allow you to receive JSON formatted data of your newsfeed. After your username and password have been verified, an API Key will be generarated for you. You will need to provide the key at the end of each of your requests to the LAN API. Here is an example request to retrieve all of items from your newsfeed with an API Key appended to the end: http://lan-social.herokuapp.com/api/newsfeed/API=yourKeyGoesHere.", example:"To see examples for GET, POST, and DELETE requests click ", link:"#/apiDocSamples"},
    {content: "When accessing our API, you may occasionaly run into errors. Our API will do its best to inform you of the nature of the problem so you may act accordingly. The problems can range from issues on our server or something simple like a syntax error in your API request. All errors will be returned in JSON formatting  with an appropriate error code and a short description of the nature of the problem.", example:"For a full listing of the error codes and their descriptions you can expect to encounter please see them ", link:"#/apiDocErrors"}
];
